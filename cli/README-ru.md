...
**Поток доставки:**
1. `git clone --depth 1 skill-hub-catalog` → `~/.skill-hub/` (локальный кеш). Защищённые файлы (`installed.json`, `bootstrap/`) сохраняются при повторном клонировании за счёт клонирования во временную директорию с последующим атомарным переносом содержимого
...
cd cli && npm test            # тесты (290 тестов)
```
