...
const COMPACT_LABELS: Record<TabName, string> = {
  catalog: 'Кат',
  installed: 'Уст',
  settings: 'Наст',
};

export const Header: React.FC<Props> = ({ activeTab, compact }) => {
  ...
  return (
    <Box borderStyle="double" borderColor={theme.border} paddingX={1}>
      <Text bold color={theme.primary}>{compact ? 's-h' : 'skill-hub'}</Text>
      ...
      {tabs.map((tab, i) => (
        <React.Fragment key={tab.id}>
          ...
          >
            [{tab.key}] {compact ? COMPACT_LABELS[tab.id] : tab.label}
          </Text>
        </React.Fragment>
      ))}
    </Box>
