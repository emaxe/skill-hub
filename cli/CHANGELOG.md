...
### Добавлено
- **Изолированный кеш каталогов (`~/.skill-hub/catalogs/<slug>-<hash>/`)** — каждый репозиторий каталога теперь клонируется в отдельную изолированную поддиректорию на основе slug репозитория и SHA256-хеша URL, что исключает конфликты при работе с несколькими каталогами.
- **Очистка устаревшего корневого кеша (`cleanLegacyRootCache`)** — автоматическое удаление старых git-файлов из корня `~/.skill-hub/` при обновлении на новую версию с сохранением `config.json`, `installed.json`, `bootstrap/` и директории `catalogs/`.
- **Игнорирование служебных файлов (`DEFAULT_IGNORE`)** — автоматическое исключение `.git`, `node_modules`, `.DS_Store`, `Thumbs.db`, `catalog.json`, `installed.json` при операциях со скиллами (копирование, валидация размера, сканирование бинарных файлов).

### Изменено
- **Гибкая валидация frontmatter при upload** — поля `version` и `author` больше не являются строго обязательными; если они не указаны, автоматически подставляется `version: 1.0.0` и `author` из `git config user.name` (или системного имени пользователя).
- **Скилл `bootstrap` исключён из upload** — добавлен в `BASE_SKILLS` для предотвращения непреднамеренной публикации в каталог.
- **Проверка origin в `updateCache`** — обнаружение смены URL удалённого репозитория и автоматическое обновление/пересоздание кеша.

## [0.1.22] — 2026-06-05

### Исправлено
- **Клонирование кеша каталога во временную директорию** — `git clone` требует абсолютно пустую целевую директорию, но кеш содержит защищённые файлы (`installed.json`, `bootstrap/`). Теперь клон выполняется во временный каталог (`os.tmpdir()`), затем содержимое переносится в целевой кеш через `moveCloneContents()` с fallback на `copy+remove` при `EXDEV` (cross-device). Защищённые файлы (`installed.json`, `bootstrap/`) сохраняются при обновлении кеша
...
### Исправлено
- **Базовые скиллы не попадают в каталог при upload** — `getUploadCandidates()` теперь фильтрует встроенные скиллы CLI (`skill-hub`, `agents-conventions`, `init-agents`, `exit-agents`) из списка кандидатов на загрузку в репозиторий каталога
- **Синхронизация: расширен список исключений** — `sync.ts` также исключает `init-agents` и `exit-agents` из missing/untracked проверок
