...
|---------------------|-------------------|
| `/skill-hub search X` | `search_extensions({query: "X", agent: "codex"})` |
| `/skill-hub install X` | `install_extension({name: "X", scope: "project", agent: "codex"})` |
| `/skill-hub install X --global` | `install_extension({name: "X", scope: "global", agent: "codex"})` |
| `/skill-hub install X --local` | `install_extension({name: "X", scope: "project", agent: "codex"})` |
| `/skill-hub remove X` | `remove_extension({name: "X", scope: "project", agent: "codex"})` |
| `/skill-hub remove X --global` | `remove_extension({name: "X", scope: "global", agent: "codex"})` |
| `/skill-hub move X --to-global` | `move_extension({name: "X", to: "global", agent: "codex"})` |
| `/skill-hub move X --to-project` | `move_extension({name: "X", to: "project", agent: "codex"})` |
| `/skill-hub list` | `list_extensions({agent: "codex"})` — показывает тип, имя, версию, scope и способ установки: `[skill-hub]` или `[manual]` |
| `/skill-hub info X` | `get_extension_info({name: "X", agent: "codex"})` |
| `/skill-hub suggest` | собери контекст (см. ниже), затем `suggest_extensions({context: "...", agent: "codex", limit: 5})` |
...
- `.codex/AGENTS.md` (если есть): первые 30 строк

Объедини всё в одну строку текста (ключевые слова и фразы через пробел).

**Шаг 2.** Вызови инструмент:
```
suggest_extensions({context: "<собранный текст>", agent: "codex", limit: 5})
```
...
```
skill-hub search <query> --agent codex
...
После установки перезапусти Codex, чтобы MCP-инструменты стали доступны.
